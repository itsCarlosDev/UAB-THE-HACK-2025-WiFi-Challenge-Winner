"""
Client lleuger per consultar l'IA d'AINA amb el context de demostració.
"""
...
def build_payload(question: str) -> dict:
    ...
    user_prompt = (
        ...
        "Si la informació necessària no apareix al context, indica que no està disponible.\n\n"
        f"--- CONTEXT ---\n{context_txt}\n"
        f"--- PREGUNTA ---\n{question.strip()}"
    )
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "de la xarxa WiFi de la UAB ni inventis dades que no apareguin al context."
                ),
            },
...
def ask_aina(question: str) -> str:
    ...
    if not API_KEY:
        raise AinaError("No s'ha configurat la variable d'entorn AINA_API_KEY.")
    ...
    if response.status_code != 200:
        raise AinaError(
            f"Error en la sol·licitud a l'API ({response.status_code}): "
            f"{response.text}"
        )
...
if __name__ == "__main__":
    print(ask_aina("Quin AP fictici té la intensitat mitjana de senyal més baixa?"))
